...
def Out_txt(LocCounter,INSFromL,Op):
    ...
    file.close()
def Out_card(LocCounter , INSFromL , Op):
    file = open('obj_program.txt','w')
    temp_H = str(hex(int(LocCounter[len(LocCounter)-2], 16) - int(LocCounter[0], 16)))
    file.write("H^"+INSFromL[0][1]+"^"+LocCounter[0]+"^"+temp_H[2:len(temp_H)]+"\n")
    for i in range(len(INSFromL)):
        if INSFromL[len(INSFromL)-1][3] in INSFromL[i][1]:
            file.write("E^"+LocCounter[i]+"\n")
        elif INSFromL[len(INSFromL)-1][3] ==' ':
            print (LocCounter[-1])
    temp_T = []
    bool_T = 0
    for i in Op:
        ...
        if i != ' ':
            if bool_T == 0:
                temp_T.append(LocCounter[Op.index(i)])
                temp_T.append(i)
                bool_T = 1
            else:
                temp_T.append(i)
            ...
            #print(len(temp_T) )
            if(len(temp_T) != 0 ):
                temp_hex = str(hex(int(LocCounter[Op.index(temp_T[-1])+1],16) - int(temp_T[0],16)))
                temp_T.insert(1,temp_hex[2:])
                file.write("T^")
                for j in temp_T:
                    file.write("^"+j)
                file.write("\n")
...
def run(filename):
  ...
  #  print(Label[:numberOfLines])
    ...
    Out_card(LocCList,INSFromL,OpCode)
    print("sucess assemble")                
